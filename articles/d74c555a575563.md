...
![firex banner](https://raw.githubusercontent.com/hummer98/firex/master/.github/banner.png)
...
firex list users --format toon   # トークン最適化（AI向け）
```

TOON（Token-Oriented Object Notation）は、LLM向けにトークン消費を40〜60%削減するコンパクトな出力形式です。
...
Firestoreのタイムスタンプを用途に応じたフォーマットで表示できます。

```bash
firex list logs --timestamp-format iso       # 2024-12-22T10:30:00.000Z（デフォルト）
...
`$fieldValue` 構文でFirestoreの特殊な値を書き込めます。

```bash
# サーバータイムスタンプ
firex set users/user1 '{"createdAt": {"$fieldValue": "serverTimestamp"}}'

# アトミックなインクリメント
firex update posts/post1 '{"viewCount": {"$fieldValue": "increment", "operand": 1}}'

# 配列に要素を追加（重複無視）
firex update users/user1 '{"tags": {"$fieldValue": "arrayUnion", "elements": ["new-tag"]}}'

# フィールドの削除
firex update users/user1 '{"obsoleteField": {"$fieldValue": "delete"}}'
```

### 9. Timestamp書き込み構文

`$timestampValue` 構文で、ISO 8601文字列をFirestore Timestampとして保存できます。

```bash
# ISO 8601文字列 → Firestore Timestamp
firex set events/event1 '{"startAt": {"$timestampValue": "2025-06-01T09:00:00Z"}}'

# タイムゾーンオフセット付き
firex set events/event2 '{"startAt": {"$timestampValue": "2025-02-18T21:00:00+09:00"}}'
```

通常の日付文字列はstring型として保存されますが、`$timestampValue` を使うことでFirestoreネイティブのTimestamp型として保存され、範囲クエリなどが正しく動作します。
