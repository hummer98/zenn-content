...
using-cmux で「1体のサブエージェントを操作する」パターンが確立できました。cmux-team はその次のステップとして、**4層のエージェント階層で複数のサブエージェントを自律的に起動・管理・統合する**フレームワークです。
...
イベント駆動の Manager、スクリプト化された Conductor 起動、pull 型通信、git worktree による隔離など、実運用で得た知見を反映したアーキテクチャです。

詳細は **[cmux-team: 4層アーキテクチャで実現するマルチエージェント開発](https://zenn.dev/hummer/articles/cmux-team-multi-agent)** をご覧ください。
