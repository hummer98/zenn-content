...
この記事は「自分でエージェントチームの基盤を書こうとしている人」向けの実装 TIPS 集です。

:::message
cmux-team 固有の話も含みますが、`ANTHROPIC_BASE_URL` プロキシやセッション管理など、Claude Code を使うエージェント基盤全般に応用できる内容です。
:::
...
Claude Code のマルチエージェント基盤を自作すると、ドキュメントの外にある挙動に頻繁に出くわします。この記事で挙げたものはほとんど実装して初めて気づいたものです。
