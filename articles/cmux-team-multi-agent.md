---
title: "cmux-team v2: AIにAIを管理させるのをやめた話"
emoji: "🏗"
type: "tech"
...
「今どのタスクがどこまで進んでいるか」を知るには、Manager の画面を読むか、ファイルを漁るしかありませんでした。Claude Code のセッションを並べるだけだと、**どのペインが何のタスクをやっているのか**が一目でわかりません。一時期はモニタに付箋を貼って管理していました。

## 新アーキテクチャ：決定論的 Daemon + セマンティック Agent

v2 では Manager を TypeScript/Bun の常駐プロセス（Daemon）に置き換えました。

```
[User] → [Master (AI)] → [Manager (Daemon/TypeScript)] → [Conductor (AI)] → [Agent (AI)]
```
...
旧版で最も不満だった「AI の処理を待たないとタスクを追加できない」問題は、ファイルベースのキューで解決しました。

```bash
# Master セッションから
bun run main.ts create-task --title "ログイン機能を実装" --priority high

# 別のターミナルからでも
bun run main.ts create-task --title "E2Eテストを追加" --priority medium

# どこからでも、何個でも、即座に追加できる
```

タスクは `.team/tasks/NNN-slug.md` としてファイルに書き出され、状態は `task-state.json` で一元管理されます。

```
.team/tasks/
├── 001-login-feature.md     # タスク内容（Markdown）
├── 002-e2e-tests.md
└── 003-api-refactor.md

.team/task-state.json        # 全タスクの状態（JSON）
{
  "001": { "status": "in_progress", "closedAt": null },
  "002": { "status": "ready", "closedAt": null },
  "003": { "status": "draft", "closedAt": null }
}
```

**タスク内容（Markdown）と状態管理（JSON）を分離**しているのがポイントです。タスクの追加はファイルを書くだけなので、どのセッションからでも、AI の処理を待たずに実行できます。Daemon は 10 秒間隔でポーリングして新しいタスクを検出し、空いている Conductor に割り当てます。
...
旧版では Conductor をタスクごとに動的に spawn していましたが、v2 では **固定 2x2 レイアウトに 3 つの常駐 Conductor** を配置する方式に変更しました。
...
Agent が知っている情報は「作業内容」と「出力先ファイル」だけです。cmux-team のアーキテクチャも、タスク管理の仕組みも、上位層の存在すら知りません。Agent はただ作業して、終わったら止まる。上位の Conductor が `cmux read-screen` で画面をポーリングして「`❯` プロンプトが出たら完了」と判定する pull 型の通信なので、Agent 側に報告プロトコルの実装は不要です。

```bash
# Conductor が Agent の完了を検出する仕組み
SCREEN=$(cmux read-screen --surface "$AGENT_SURFACE")
# "❯" があり "esc to interrupt" がない → 完了
# "❯" があり "esc to interrupt" がある → まだ実行中
# surface が存在しない → クラッシュ
```

つまり、プリセットの Implementer / Researcher / Tester を使う必要はなく、**あなたが普段使っている Claude Code のセッションや CLAUDE.md の設定をそのまま Agent として組み込めます**。Gemini CLI でも動きます。Conductor が見ているのは「ターミナルの画面にプロンプトが表示されたかどうか」だけだからです。
...
cmux-team v2 は **オーケストレーション層を決定論的なコードに置き換える** ことで、この問題を構造的に排除しました。AI が判断すべき部分（タスクの分解、コードの実装）だけを AI に任せます。
...
Conductor が 3 つ固定なので、並列度は最大 3 です。動的にスケールさせたい場面では制約になります。ただし実運用では、3 並列でも十分なことが多く、むしろ **並列度が予測可能** であることのメリットが大きいです。トークン消費も爆発しにくい。

### ポーリングの遅延

Daemon は 10 秒間隔でタスクをスキャン、Conductor は 30 秒間隔で Agent を監視します。イベント駆動と比べると最大で数十秒の遅延が生じます。ただし、AI エージェントのタスクは数分〜数十分かかるものが多いので、この遅延が問題になることはほぼありません。

### セットアップの複雑さ

[cmux](https://cmux.dev) + [Claude Code](https://docs.anthropic.com/en/docs/claude-code) + Bun が前提です。SaaS 型のように URL にアクセスして即使えるものではありません。その代わり、環境の自由度と透明性を得ています。
...
cmux-team v2 で得た最大の学びは、**AI にやらせるべきことと、コードでやるべきことの境界線は思ったより明確だ**ということです。
